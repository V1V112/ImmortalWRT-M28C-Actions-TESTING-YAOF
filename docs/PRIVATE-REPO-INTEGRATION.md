...
| 参数 | 说明 |
| --- | --- |
| `custom_config_repo` | 私人配置仓库 URL，例如 `https://github.com/YOUR_USERNAME/openwrt-configs.git` |
| `custom_config_branch` | 配置仓库分支、标签或可拉取引用，默认 `main` |
| `custom_config_required` | 设为 `true` 时，私人仓库拉取失败会直接终止构建；默认 `false` 会回退到本地 `files` |

私人仓库可以包含 `files/` 目录，也可以让仓库根目录直接作为 overlay 根目录。项目仍会保持本地 `files` 最高优先级。
...
```
/usr/bin/
├── app1-aarch64          # 默认保留选中候选文件名
└── app2-aarch64
```

如需安装为 `/usr/bin/app1` 和 `/usr/bin/app2`，可设置 `OVERLAY_BIN_NAME_MODE=preferred`。
...
- `CUSTOM_CONFIG_REQUIRED`: 拉取失败时是否终止构建（默认：false）
