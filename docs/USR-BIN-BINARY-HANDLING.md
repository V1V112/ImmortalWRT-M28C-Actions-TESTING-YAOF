...
默认行为保持向后兼容：安装时使用被选中的候选文件名，例如 `myapp-aarch64` 会安装为 `/usr/bin/myapp-aarch64`。只有当候选文件名是 `bin`、`linux`、`arm64`、`aarch64`、`release` 这类泛用名称时，才会改用目录名或压缩包名。

如果希望始终使用目录名或压缩包名作为最终文件名，可以设置：
...
1. **stage-overlay.sh** - 本地文件处理
   - 函数：`stage_usr_bin()`，并调用公共二进制处理函数
   - 位置：`scripts/stage-overlay.sh`

2. **fetch-custom-config.sh** - 私人仓库处理
   - 函数：拉取配置、合并文件，并调用公共二进制处理函数
   - 位置：`scripts/fetch-custom-config.sh`

3. **overlay-bin-common.sh** - 公共二进制处理
   - 函数：`install_detected_binary()`, `candidate_score()` 等
   - 位置：`scripts/overlay-bin-common.sh`
...
5. 默认安装 `myapp-aarch64` 到 `/usr/bin/myapp-aarch64`；如果设置 `OVERLAY_BIN_NAME_MODE=preferred`，则安装为 `/usr/bin/myapp`
